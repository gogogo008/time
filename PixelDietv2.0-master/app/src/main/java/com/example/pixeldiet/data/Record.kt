...
data class UserProfileEntity(
    ...
    val friendsJson: String = "[]" // 친구 UID 리스트를 JSON으로 저장
) {
    // 친구 리스트를 쉽게 접근할 수 있는 헬퍼
    fun getFriends(): List<String> = Gson().fromJson(friendsJson, object : TypeToken<List<String>>() {}.type)
    fun setFriends(friends: List<String>): UserProfileEntity =
        copy(friendsJson = Gson().toJson(friends))
}
