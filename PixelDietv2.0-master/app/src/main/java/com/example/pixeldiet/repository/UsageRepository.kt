...
object UsageRepository {
    ...
    // ---------------- 추적 앱 관리 ----------------
    fun loadTrackedApps() = CoroutineScope(Dispatchers.IO).launch {
        val apps = db.trackedAppDao().getAllTrackedAppsOnce()
    ...
    suspend fun updateGoalTime(packageName: String, goalTime: Int) {
        val app = TrackedAppEntity(packageName, goalTime)
        db.trackedAppDao().insertOrUpdate(app)

        // trackedAppsFlow 갱신
        val currentList = _trackedApps.value.toMutableList()
        currentList.removeAll { it.packageName == packageName }
        currentList.add(app)
        _trackedApps.value = currentList

        // appUsageListFlow 갱신
        updateGoalTimes(mapOf(packageName to goalTime))
    }
    ...
    // ---------------- DailyUsage 불러오기 ----------------
    fun observeDailyUsage(uid: String, from: String, to: String): LiveData<List<DailyUsage>> {
        return db.dailyUsageDao()
            .getUsageInRange(uid, from, to)
            .map { list -> list.map { it.toDailyUsage() } }
            .asLiveData()
    }
